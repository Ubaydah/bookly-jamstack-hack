import { Box, Text, Flex, Divider, Spinner, Input, IconButton, InputGroup, InputLeftAddon } from "@chakra-ui/react";
...
const Gallery = () => {
  ...
  const handleSearch = async (e) => {
    ...
    const data = await axios.post(`/api/book/search`, { value });
    ...
    <Flex minHeight={"100vh"} flexDirection="column" bg="gray.100">
      ...
      <Box p={{ base: "5", md: "12" }}>
        <Box w="fit-content" mx="auto">
          ...
          <InputGroup mt="5" border="1px solid teal" borderRadius="10px">
            <InputLeftAddon children={<SearchIcon color="white" />} bg="teal" />
            <Input
              placeholder="Search"
          ...
          <Flex gap="6" wrap={"wrap"} justifyContent="start" flexDirection={{ base: "column", md: "row" }} marginTop="12">
            {books?.map((item: any, index: number) => (
              ...
              >
                ...
                <Divider mt="2" variant={"solid"} color="teal.100" />
                <Box bg="white" px="8" h="80%" py="6">
